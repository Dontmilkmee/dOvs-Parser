...
let init_testsets featureSetMap only overwrite fs  =     
  DefaultTesting.defaultTestsForPhases
     { use_filter = Option.map Str.regexp only
     ; overwrite 
     ; feature_map = featureSetMap
     ; feature_sets = Option.map (fun s -> Str.regexp s ) fs
     } 
     defaultPhasesToCheck 

let init_testsets_t fs = Term.(const (init_testsets fs)
    $ only_flag $ overwrite_flag $featureset_flag)

let dummy_testsets_t = Term.(const (fun _ _ _-> ()) 
    $ only_flag $ overwrite_flag $featureset_flag)  
